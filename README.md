# Compatibility
All black & white classic iPods (from 1G to 4G), but I could test those themes only with a 4G.

# Motivations
Get the same experience as on my iPod Mini and the same look & feel by adapting the themes I've made for it but for classic iPods which have a larger screen.
...
- The realtime volume viewer is now disabled automatically for OPUS files in order to save some precious CPU cycles. Old iPods are struggling hard with this format, but this format is very useful to encode audiobooks at very low bitrate (32 to 40kbps) but with high quality.
